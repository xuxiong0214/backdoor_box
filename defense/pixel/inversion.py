...
class PixelBackdoor:
    def __init__(self,
                 ...
                 steps=25,              # number of steps for inversion
        ...
        self.num_classes = box.num_classes
        self.steps = steps
    ...
    def generate(self, opt, cln_trainloader):
        # store best results
        ...
        for step in range(self.steps):
            ...
            loss_list = []
            acc_list = []
            for x_batch, _, _, y_batch in cln_trainloader:
                # map pattern variables to the valid range

                x_batch = self.denormalizer(x_batch)
                x_batch = x_batch.to(self.device)
                y_batch = torch.ones([x_batch.shape[0]], device=self.device, dtype=torch.int64) * opt.now_target
