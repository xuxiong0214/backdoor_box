...
def get_model(opt):
    ...
    if opt.model == "resnet18":
        # model = resnet.ResNet18(num_classes=opt.num_classes).to(opt.device)
        model = resnet_imagenet.resnet18().to(opt.device)
    if opt.model == "vgg16":
        model = vgg.VGG("VGG16", num_classes=opt.num_classes).to(opt.device)
    if opt.model == "densenet":
        model = densenet.DenseNet121(num_classes=opt.num_classes).to(opt.device)
    if opt.model == "vit":
        model = vit.ViT(
...
def train(train_transform, model, optimizer, scheduler, train_dl, epoch, opt, residual_list_train):
    print(" Train:")
    ...
    for batch_idx, (inputs,_,_, targets) in enumerate(train_dl):
        optimizer.zero_grad()
...
def eval(
    ...
    for batch_idx, (inputs,_,_, targets) in enumerate(test_dl):
        with torch.no_grad():
            inputs, targets = inputs.to(opt.device), targets.to(opt.device)
            bs = inputs.shape[0]
            total_sample += bs
...
def main(target_label):
    opt = config.get_arguments().parse_args()
    # opt.target_label = target_label
    # opt.checkpoints = f"./checkpoints/{opt.dataset}-bppattack-{opt.model}-target{target_label}"
    opt.checkpoints = f"./checkpoints/{opt.dataset}-bppattack-{opt.model}-targetall"
    ...
    opt.ckpt_path = os.path.join(opt.checkpoints, "state_dict.pt.tar")
    ...
    for epoch in range(epoch_current, opt.n_iters):
        ...
        train(train_transform,model, optimizer, scheduler, train_dl, epoch, opt, residual_list_train)
