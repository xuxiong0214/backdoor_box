...
class BADCIFAR(cifar.CIFAR):
    def __init__(self, pidx, tlabel, path, train, train_type=None, tf=None) -> None:
        ...
        self.pidx = pidx
        self.tlabel = tlabel
        self.trigger_info = torch.load("../../attack/badnets/trigger_info/cifar_default_trigger.pth")
        self.totensor = transforms.Compose([transforms.ToTensor()])
        self.bd_transform = transforms.Compose([Addtrigger(self.trigger_info["mask"], self.trigger_info["ptn"])])
...
bad_trainset = BADCIFAR(pidx=train_pidx, tlabel=tlabel, path="../../datasets/cifar10", train=True, train_type=1, tf=train_tf)

sptn = bad_trainset.trigger_info["ptn"]
smask = bad_trainset.trigger_info["mask"]


cln_testset = cifar.CIFAR(path="../../datasets/cifar10", train=False, tf=test_tf)
test_pidx = np.ones(len(cln_testset.datas))
bad_testset = BADCIFAR(pidx=test_pidx, tlabel=tlabel, path="../../datasets/cifar10", train=False, tf=test_tf)
...
def test():
    ...
    with torch.no_grad():
        ...
        save_path = f"../../checkpoints/cifar-badnets-vit-target{tlabel}"
